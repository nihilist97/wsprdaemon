        ...
        const int tuning_freq_hz )
{
    ...
    fprintf( stderr,"create_session(): type %d ssrc %d\n", rtp->type, rtp->ssrc );
    ...
    // it actually does not work for a channel not defined in rtp.c
    // radiod (main.c) add a channel to the PT table at runtime, but the wd-record
    // process can only get the type from rtp info, and the channel identified by the type
    // does not exists in the PT table known by wd-record.
    if ( sp->channels == 0 )  {
        fprintf( stderr,"create_session(): ERROR: channel info not found in PT_table (rtp.c)\n" );
        return NULL; 
    }
