...
function archive_24hour_wavs_to_data_dir() {
    ...
    local reporter_id=${reporter_info%_*}       ### Chop off the _GRID to get the WSPR reporter id: N0HAQ
    ...
    for receiver_dir in ${receiver_dir_list[@]} ; do
        # ./20250312/N0HAQ_OL62ma/KA9Q_0_WWV
        local receiver_info="${receiver_dir##*/}"           # KA9Q_0_WWV
        local receiver_name="${receiver_info%@*}"           # KA9Q_0_WWV ?
        local pswsnetwork_info="${receiver_info#*@}"

        wd_logger 1  "Checking and cleaning up the band directories for receiver ${receiver_dir}"
        wd_logger 2  "date: ${wav_date}- site: ${reporter_id} - receiver_name: $receiver_name "
        ...
        if [[ ${wav_file_count} -eq 0 ]]; then
            wd_logger 1 "WARNING: no wav files found or created for any bands, so skip archiving this receiver/band"
            continue
        fi
    ...
    touch "${reporter_archive_complete_file_name}"
    wd_logger 1  "Archiving was successful, so create '${reporter_archive_complete_file_name}'"
}
...
#    if [[ ${LAST_HHMM} == "0" || ${current_hhmm} != ${LAST_HHMM} && ${current_hhmm} == ${GRAPE_UPLOAD_START_HHMM} ]]; then
    ...
    if  [[ ${rc} -eq 0 ]]; then
        ...
        wd_logger 1 "There were ${rc} yesterday 24h.wav files created"
    fi
# following codes create *all* date files
